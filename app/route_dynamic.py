...
def Supported_Countries():
    ...
    if (
        ...
        cfgserv.app_logger.debug(", Session ID: " + session["session_id"] + ", " + "Authorization selection, Type: " + form_country)
...
def red():
    ...
    if session["country"] == "LV":
        # create header mapping with authorization header and content type 
        # how to get base64 from string
        client_data = cfgcountries.supported_countries[session["country"]]["oidc_auth"]["client_id"]+":"+cfgcountries.supported_countries[session["country"]]["oidc_auth"]["client_secret"]
        byte_string = client_data.encode('utf-8')
...
def credentialCreation(credential_request, data, country):
    ...
    for credential in credential_request:
        ...
        if "credential_identifier" in credential:
            ...
            if format not in ("mso_mdoc", "vc+sd-jwt"):
                return {
                    "error": "invalid_credential_request",
                    "error_description": f"invalid format '{format}'",
                }
        ...
        if country == "FC":
            ...
            if country == "LV":
                for attribute in data:
                    if attribute == "first_name":
                        form_data["given_name"] = data[attribute]
                    elif attribute == "last_name":
                        form_data["family_name"] = data[attribute]
                    else:
            ...
            form_data["birth_date"] = "1990-10-31"
            form_data["age_over_18"] = True
