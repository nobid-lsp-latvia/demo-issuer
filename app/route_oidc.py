...
def do_response(endpoint, req_args, error="", **args) -> Response:
    ...
    cfgservice.app_logger.debug("do_response: {}".format(info))
    ...
    if error:
        if _response_placement == "body":
            cfgservice.app_logger.debug("Error Response: {}".format(info["response"]))
            ...
            cfgservice.app_logger.debug("Redirect to: {}".format(info["response"]))
            resp = redirect(info["response"])
    else:
        if _response_placement == "body":
            cfgservice.app_logger.debug("Response: {}".format(info["response"]))
...
def token():
    ...
    if req_args["grant_type"] == "authorization_code":
        ...
        # response = response.json()
        cfgservice.app_logger.debug("Token response: " + str(response.json()))
...
def service_endpoint(endpoint):
    # _log = current_app.logger
    cfgservice.app_logger.debug('At the "{}" endpoint'.format(endpoint.name))
    ...
    cfgservice.app_logger.debug(f"http_info: {http_info}")
