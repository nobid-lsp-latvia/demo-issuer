...
def formatter(data, un_distinguishing_sign, doctype, format):
    ...
    for request in credentialsSupported:
        if (
            ...
            # try to fix the driving_privileges attribute error if they are come from csdd
            if "driving_privileges" in attributes_req:
                privileges = data["driving_privileges"]
                if isinstance(privileges, str):
                    # Parse only if it's a string
                    json_priv = json.loads(privileges)
                else:
                    # Use directly if it's already parsed
                    json_priv = privileges
