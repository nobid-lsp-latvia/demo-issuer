...
# from preauthorization import generate_offer, generate_preauth_token

custom = Blueprint("custom", __name__, url_prefix="/")
CORS(custom)  # enable CORS on the blue print

# custom code, puts together /credential_offer, /preauth_form and /form_authorize_generate
# example json:
# {
#     "credentialIds": [
...
#         "given_name": "TestUser",
#         "birth_date": "2024-07-03",
#         "estimated_issuance_date": "2024-11-22",
#         "estimated_expiry_date": "2025-02-20",
#         "issuing_country": "FC",
...
@custom.route("/generate_credential_offer", methods=["POST"])
def generate_credential_offer():
    data = request.json
    form = data["form"]
    # 1 credential_offer
    credentials_id = data["credentialIds"]
    # session["credentials_id"] = credentials_id
    return_in_html =data["returnInHtml"]
    credential_offer_uri = data["credentialOfferURI"]
    ...
    form_dynamic_data[user_id] = form.copy()
    ...
    # 3 form_authorize_generate

    # returns in old front
    if return_in_html:
        # prepare sessions
        session["credentials_id"] = credentials_id
        ...
        return generate_offer(form)

    # return as json data

    pre_auth_code = generate_preauth_token(data=form, authorization_details=authorization_details)

    tx_code = random.randint(10000, 99999)
    transaction_id = generate_unique_id()

    transaction_codes.update({transaction_id: {"pre_auth_code": pre_auth_code, "tx_code": str(tx_code),
                                               "expires": datetime.now() + timedelta(
                                                   minutes=cfgservice.tx_code_expiry)}})
...
# here is copy from preauthorization.py
...
def generate_offer(data):
    ...
    return render_template(
        ...
        wallet_dev=wallet_url + "?code=" + transaction_id + "&tx_code=" + str(
            tx_code) + "&credential_offer=" + json.dumps(credential_offer),
        url_data=uri,
        tx_code=tx_code,
        qrcode=qr_img_base64,
    )
...
def generate_preauth_token(data, authorization_details):
    ...
    data.update({"issuing_country": "LV"})
    ...
    cfgservice.app_logger.debug(logText)
